    ...
    <?php for($i = 1; $i<=date('t'); $i++):?>
        ...
        <div class="b-planed-payments-block">
            <div class="b-date-nav">
                <span class="b-day-name b-left"><?= PayPlanned::getWeekDays()[($i%7 === 0) ? ($i%7)+1 : $i%7]?></span>
                <span class="b-date-name b-right"><?= Yii::$app->formatter->asDate(date('Y-m-d', strtotime('first day of this month') + 60*60*24*($i-1)), 'php:d').' '.PayPlanned::getMonthNames()[date('n')]?></span>
            </div>
